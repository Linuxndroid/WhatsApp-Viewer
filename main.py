...
def query_messages_from_table_message(con: Connection, key_remote_jid: str, contacts: Dict[str, Optional[str]]) -> List[Message]:
    cur = con.cursor()
    # --- FIXED: Added a new join for chat_jid and changed the WHERE clause ---
    query = """  
            SELECT
                m.timestamp,
...
def query_all_chats(db_path: str, contacts: Dict[str, Optional[str]]) -> List[Chat]:
    ...
    # --- FIXED: Changed query to JOIN chat_view with jid table ---
    query = """
            SELECT 
            ...
            LEFT JOIN jid AS j ON cv.jid_row_id = j._id
            WHERE cv.sort_timestamp IS NOT NULL 
            ORDER BY cv.sort_timestamp DESC
            """
